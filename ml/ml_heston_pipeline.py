...
class HestonSurfaceSurrogate(nn.Module):
    def __init__(self, n_outputs: int):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(5, 256), nn.SiLU(), nn.Linear(256, 256), nn.SiLU(),
                                 nn.Linear(256, 256), nn.SiLU(), nn.Linear(256, n_outputs))
...
def calibrate_surrogate(observed, model, x0, device="cpu"):
    ...
    result = minimize(objective, np.asarray(x0, dtype=np.float64), jac=True, method="L-BFGS-B",
                      options={"maxiter": 150, "ftol": 1e-10})
    params = unconstrained_to_heston(torch.tensor(result.x, dtype=torch.float32, device=device))
...
def generate_dataset(path="ml/data/heston_surrogate.npz", n=10000, outputs=84, seed=42):
    ...
    surfaces = (params[:, 1:2] + params[:, 4:5] + 0.15 * params[:, 3:4] * k[None, :]
                + 0.03 * params[:, 2:3] * k[None, :] ** 2).astype(np.float32)
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(path, params=params, surfaces=surfaces, raw=raw, seed=seed)


def train(path="ml/data/heston_surrogate.npz", output="ml/artifacts/heston_surrogate.pt", epochs=50, batch_size=256):
    data = np.load(path)
...
if __name__ == "__main__":
    ...
    if args.command == "generate": generate_dataset(args.path)
    elif args.command == "train": train(args.path, args.output)
    else: print(evaluate(args.path, args.output if Path(args.output).exists() else None, args.smoke_test))
