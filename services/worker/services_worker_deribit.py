"""Optional Deribit ingestion worker; demo mode remains available without credentials."""
...
class DeribitProvider:
    ...
    async def get_chain(self, underlying: str) -> list[OptionQuote]:
        async with httpx.AsyncClient(timeout=15) as client:
            instruments = await client.get(f"{self.base_url}/public/get_instruments",
                                           params={"currency": underlying.upper(), "kind": "option", "expired": "false"})
            instruments.raise_for_status()
            rows = instruments.json()["result"]
            ticker_rows = []
            for instrument in rows:
                response = await client.get(f"{self.base_url}/public/ticker",
        ...
        for instrument, ticker in ticker_rows:
            expiry = datetime.fromtimestamp(instrument["expiration_timestamp"] / 1000, timezone.utc)
            quotes.append(OptionQuote(
                instrument=instrument["instrument_name"], underlying=underlying.upper(), timestamp=now,
                expiry=expiry, strike=float(instrument["strike"]),
                option_type="call" if instrument["option_type"] == "call" else "put",
                bid=ticker.get("best_bid_price"), ask=ticker.get("best_ask_price"), mark=ticker.get("mark_price"),
                underlying_price=float(ticker.get("underlying_price") or ticker.get("index_price")),
                interest_rate=0.0))
        return quotes
...
async def snapshot(underlying: str = "BTC") -> int:
    ...
    # Persistence is intentionally injected at deployment time; this worker only owns acquisition.
    return len(quotes)
